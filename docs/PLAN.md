...
| **R** | **Notes**           | Free-form admin context the agent can't infer from structured fields — *"gift from Sarah"*, *"bought for Iceland trip 2024"*, *"stored at the cabin"*, *"backup pair"*, *"custom fit for narrow heel"*. Empty by default; never written by parsers / classifiers. Read by the agent as additional context when answering questions. |
...
**eReceipt enrichment (added 2026-05-17):** `lib/parsers/rei-product-lookup.ts` (Sonnet 4.6 + `web_search`) resolves the POS register abbreviation in each eReceipt item (e.g. `LoungerDLChairMesh`, `Trlmade Rain Pant`, `Renegade Recon`) to the canonical brand + marketing name. Wired into `parseEmail` in `apps/cron/pipeline.ts` so every eReceipt item runs through enrichment before classification. Direct REI page scraping isn't available — Cloudflare hard-blocks non-browser User-Agents — so we use Anthropic's commercial web_search infrastructure. `scripts/enrich-rows.ts` backfills the same enrichment over existing eReceipt rows in the sheet (filter pattern `^S\d+-T\d+$`).
