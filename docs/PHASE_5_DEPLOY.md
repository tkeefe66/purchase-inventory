...
- **RIDB doesn't expose structured camping metadata.** Season window, lead-time-days, fee amount, amenities — none are first-class fields on the Rec.gov RIDB v1 facility schema. T9 metadata-refresh falls back to defaults: `leadTimeDays ?? 180`, `feeUSD ?? 0`, `amenities = []`. **Net effect:**
  - 7-day + release-moment nudges fire correctly (use leadTimeDays default).
  - Season-opener nudges fire on the **default window** (mid-May / mid-October, set in `lib/reccgov/seasons.ts`'s `DEFAULT_SEASON`). Refine per-site by adding entries to `FACILITY_SEASON_OVERRIDES` as you learn actual open/close dates.
