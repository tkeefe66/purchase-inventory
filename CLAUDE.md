...
| Web UI v1 | Read-only dashboard shipped 2026-05-17 — Next.js 14 App Router at repo root `/app`. Three pages: `/` filterable items table, `/spending` Recharts (4 charts), `/needs-review` parse-failure viewer. HTTP Basic Auth via `middleware.ts` (`WEB_USER` / `WEB_PASSWORD` env vars). Deployed as Web service in Railway Purchase-Inventory project via `railway.web.json`. Editing deferred. |
