...
const HEADER = `=== ACTIVE PHOTOGRAPHY INVENTORY ===
Format: [id] | Year | [Brand] Item (Color, Size) | $price [Category/Sub-Category, Type] | <productUrl or empty>
...
export function serializeCompact(rows: readonly MasterRow[]): CompactView {
  const filtered = rows.filter((r) => r.domain === 'Photography' && r.status === 'active');
...
function formatRow(row: MasterRow): string {
  ...
  return `[${id}] | ${row.year} | ${brandPart}${row.itemName}${colorSize} | $${price} [${cat}, ${row.type}] | ${row.productUrl}`;
}
