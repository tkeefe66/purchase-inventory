...
// Module-scoped Nominatim rate-limit state and cache — shared by both
// the standalone geocode() export and the createWeatherClient closure.
...
/** Reset module-scoped Nominatim state. For use in tests only. */
...
/**
 * Standalone geocoder. Shares the module-scoped Nominatim rate-limiter and
 * cache with createWeatherClient so both paths obey the ≥1s/req policy.
...
export function createWeatherClient(opts: WeatherClientOptions): WeatherClient {
  ...
  return {
    async getForecast(input) {
      const { lat, lon, name } = await doGeocode(input.location, fetchImpl, now, sleep);
