...
function handleHelp(deps: HandlerDeps): string {
  ...
  return [
    `*Purchase-Inventory bot — commands*`,
    ``,
    `*Domain switching*`,
    `/outdoor — switch to outdoor mode`,
    `/photo — switch to photography mode`,
    `/who — show current mode`,
    `/outdoor <msg> — one-off message to outdoor agent`,
    `/photo <msg> — one-off message to photography agent`,
    ``,
    `*Capture*`,
    ...
    ``,
    `*Photography*`,
    `/skills — full skills tree with progress`,
    `/track <branch> — browse a branch (operating-camera, seeing, editing, printing)`,
    `/next — next suggested topic`,
    `/learn <topic> — read theory for a topic`,
    `/start <topic> — begin an assignment`,
    `/active — show current assignment`,
    `/skip — skip current assignment`,
    `/plan [duration] — suggested learning plan`,
    `Send a photo to submit for grading (compressed OK).`,
    ``,
    `*Camping*`,
    ...
    ``,
    `*Status*`,
    `/scan — force the bot to scan your email for new purchases right now`,
    ...
    ``,
    `Or just chat — messages go to the active domain's agent.`,
  ].join('\n');
}
